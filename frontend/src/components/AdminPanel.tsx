...
import { UserPlus, Trash2, Users, LogOut, Search, BarChart3, Clock, Eye } from "lucide-react";
...
export default function AdminPanel() {
  ...
  // Build visit count map for member list
    ...
    <div className="h-[100dvh] w-screen bg-[var(--color-bg-primary)] bg-dot-grid overflow-hidden flex flex-col">
      ...
      <header className="glass-strong h-14 border-b border-[var(--color-border)] flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-sm font-bold text-[var(--color-text-primary)]">관리자 패널</span>
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="glass rounded-2xl border border-white/10 p-4 text-center">
                  ...
                  <p className="text-[10px] text-[var(--color-text-muted)] mt-1">총 방문 수</p>
                </div>
                <div className="glass rounded-2xl border border-white/10 p-4 text-center">
                  <p className="text-2xl font-black text-[#22c55e]">{visitStats.stats.length}</p>
                  <p className="text-[10px] text-[var(--color-text-muted)] mt-1">방문한 유저</p>
                </div>
                <div className="glass rounded-2xl border border-white/10 p-4 text-center">
                  <p className="text-2xl font-black text-[#8b5cf6]">{data.members.length}</p>
                  <p className="text-[10px] text-[var(--color-text-muted)] mt-1">총 멤버</p>
                </div>
              </div>
              ...
              <div className="glass rounded-2xl border border-white/10 overflow-hidden" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
                {/* Table header */}
                <div className="grid grid-cols-[1fr_80px_100px] sm:grid-cols-[1fr_100px_140px] px-4 py-2.5 border-b border-[var(--color-border)] bg-white/3">
                  <span className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-wider">유저</span>
                  <span className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-wider text-center">방문 수</span>
                  <span className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-wider text-right">마지막 방문</span>
                </div>

                <div className="max-h-[55vh] overflow-y-auto divide-y divide-[var(--color-border)]/30">
                  {filteredVisits.length === 0 ? (
                    <div className="px-4 py-8 text-center">
                      <Eye size={24} className="text-[var(--color-text-muted)] mx-auto mb-2 opacity-40" />
                      <p className="text-xs text-[var(--color-text-muted)]">
                        {search ? "검색 결과가 없습니다" : "아직 방문 기록이 없습니다"}
                      </p>
                    </div>
                  ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-center">
                            ...
                            <span className="text-[10px] text-[var(--color-text-muted)] ml-0.5">회</span>
                          </div>
                          <div className="text-right flex items-center justify-end gap-1">
                            <Clock size={10} className="text-[var(--color-text-muted)]" />
                            <span className="text-[11px] text-[var(--color-text-muted)]">
                              {timeAgo(stat.last_visit)}
                            </span>
                          </div>
                        </div>
                      );
